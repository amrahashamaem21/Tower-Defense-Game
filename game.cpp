...
static const float POP_Y = 384.f;  
static const float POP_W = 340.f;
static const float POP_H = 440.f;

Game::Game(): window(sf::VideoMode(1366, 768), "Tower Defense", sf::Style::Close | sf::Style::Resize),
    wave(&gold, &lives),
    ...
    showEnemyCards(true)
{
    ...
    loadAssets();
    if (!bgMusic.openFromFile("Sounds/game_bg_music.ogg"))
    {
    ...
    bgMusic.play();
    wave.set_way_points(map.getWaypoints(), map.getWaypointCount(),
        map.getEntryPoint(), map.getExitPoint());
    wave.set_up_wave(1,selectedLevel);
    // Hover cell
...
void Game::loadAssets()
{
    ...
    victorySoundBuffer.loadFromFile("Sounds/game_win.wav");
    victorySound.setBuffer(victorySoundBuffer);
    defeatSoundBuffer.loadFromFile("Sounds/game_defeat.ogg");
...
{
    // Circle setup
    levelBubbleCircle[i].setRadius(38.f);
    levelBubbleCircle[i].setOrigin(38.f, 38.f);
    levelBubbleCircle[i].setFillColor(bc[i]);
    levelBubbleCircle[i].setOutlineColor(sf::Color::White);
    levelBubbleCircle[i].setOutlineThickness(3.f);
    levelBubbleCircle[i].setPosition(bx[i], by[i]);
    // Text setup
    levelBubbleNumText[i].setFont(font);
    levelBubbleNumText[i].setString(std::to_string(i + 1));
    levelBubbleNumText[i].setCharacterSize(26);
    levelBubbleNumText[i].setFillColor(sf::Color::White);
    sf::FloatRect nb = levelBubbleNumText[i].getLocalBounds();
    levelBubbleNumText[i].setPosition(
        bx[i] - nb.width / 2.f - 2, by[i] - nb.height / 2.f - 8);
    // Data setup
    levelBubbleLocked[i] = (i >= unlockedLevels);
    levelBubbleX[i] = bx[i];
    levelBubbleY[i] = by[i];
}
    // CHARACTER PANEL
    ...
    for (int i = 0; i < NUM_CHARS; i++)
    {
        ...
        enemyCardBg[i].setSize(sf::Vector2f(205, 215));
        enemyCardBg[i].setPosition(cx, cy);
        enemyCardBg[i].setFillColor(enemyBgColors[i]);
        enemyCardBg[i].setOutlineColor(enemyOutlineColors[i]);
        enemyCardBg[i].setOutlineThickness(2.5f);

        enemyCardTex[i].loadFromFile(enemyTexPaths[i]);
        enemyCardSpr[i].setTexture(enemyCardTex[i]);
        float targetSize = 85.f;
        if (i == 4)
            targetSize = 70.f;  
        float sScale = targetSize / std::max(
            (float)enemyCardTex[i].getSize().x,
            (float)enemyCardTex[i].getSize().y);
        enemyCardSpr[i].setScale(sScale, sScale);
        sf::FloatRect eb = enemyCardSpr[i].getLocalBounds();
        enemyCardSpr[i].setOrigin(eb.width / 2.f, eb.height / 2.f);
        enemyCardSpr[i].setPosition(cx + 102, cy + 88);
        enemyCardNameTxt[i].setFont(font);
        enemyCardNameTxt[i].setString(enemyNames[i]);
        enemyCardNameTxt[i].setCharacterSize(14);
        enemyCardNameTxt[i].setFillColor(sf::Color(20, 20, 40));
        enemyCardNameTxt[i].setPosition(cx + 8, cy + 8);
        enemyCardDescTxt[i].setFont(font);
        enemyCardDescTxt[i].setString(enemyDescs[i]);
        enemyCardDescTxt[i].setCharacterSize(10);
        enemyCardDescTxt[i].setFillColor(sf::Color(30, 30, 60));
        enemyCardDescTxt[i].setPosition(cx + 8, cy + 148);
    }
    ...
    for (int i = 0; i < NUM_CHARS; i++)
    {
        ...
        towerCardBg[i].setSize(sf::Vector2f(205, 215));
        towerCardBg[i].setPosition(cx, cy);
        towerCardBg[i].setFillColor(towerBgColors[i]);
        towerCardBg[i].setOutlineColor(towerOutlineColors[i]);
        towerCardBg[i].setOutlineThickness(2.5f);

        towerCardTex[i].loadFromFile(towerTexPaths[i]);
        towerCardSpr[i].setTexture(towerCardTex[i]);
        float targetSize = 85.f;
        float sScale = targetSize / std::max(
            (float)towerCardTex[i].getSize().x,
            (float)towerCardTex[i].getSize().y);
        towerCardSpr[i].setScale(sScale, sScale);
        sf::FloatRect tb = towerCardSpr[i].getLocalBounds();
        towerCardSpr[i].setOrigin(tb.width / 2.f, tb.height / 2.f);
        towerCardSpr[i].setPosition(cx + 102, cy + 88);

        towerCardNameTxt[i].setFont(font);
        towerCardNameTxt[i].setString(towerNames[i]);
        towerCardNameTxt[i].setCharacterSize(14);
        towerCardNameTxt[i].setFillColor(sf::Color(20, 20, 40));
        towerCardNameTxt[i].setPosition(cx + 8, cy + 8);

        towerCardDescTxt[i].setFont(font);
        towerCardDescTxt[i].setString(towerDescs[i]);
        towerCardDescTxt[i].setCharacterSize(10);
        towerCardDescTxt[i].setFillColor(sf::Color(30, 30, 60));
        towerCardDescTxt[i].setPosition(cx + 8, cy + 148);
    }
    // START BUTTON
...
void Game::setupDetailPopup(int index, bool isEnemy)
{
    ...
    if (isEnemy)
    {
        ...
        detailSprite = enemyCardSpr[index];
    }
    else
    {
        ...
        detailSprite = towerCardSpr[index];
    }
    ...
    // Big sprite centered
    detailSprite.setScale(1.f, 1.f);
    float popupScale = 0.42f;
    if (isEnemy && index == 4)  // Jelly
        popupScale = 0.18f;

    detailSprite.setScale(popupScale, popupScale);
...
void Game::processEvents()
{
    ...
    while (window.pollEvent(event))
    {
            ...
            event.mouseButton.button == sf::Mouse::Left)
        {
            ...
            {
                sf::RectangleShape* cards = showEnemyCards ? enemyCardBg : towerCardBg;
                for (int i = 0; i < NUM_CHARS; i++)
                {
                    if (cards[i].getGlobalBounds().contains(mx, my))
                    {
                ...
                for (int i = 0; i < NUM_LEVELS; i++)
                {
                    if (levelBubbleLocked[i])
                    {
                        continue;
                    }
                    float dx = mx - levelBubbleX[i];
                    float dy = my - levelBubbleY[i];
                    if (dx * dx + dy * dy <= 38.f * 38.f)
                    {
                        selectedLevel = i + 1;
                        for (int j = 0; j < NUM_LEVELS; j++)
                            levelBubbleCircle[j].setOutlineThickness(3.f);
                        levelBubbleCircle[i].setOutlineThickness(6.f);
                    }
                }
...
void Game::update(float dt)
{
    ...
    {
        hud.update();
        if (wave.has_started() && state == STATE_PLAYING)
        {
            wave.update(dt);
            towerAttack(dt);
            checkGameState();
...
void Game::render()
{
    ...
    else if (state == STATE_PLAYING ||state == STATE_VICTORY ||state == STATE_GAME_OVER)      
    {
        renderPlaying();
    }
    else if (state == STATE_WAVE_CLEAR)      
...
void Game::renderLevelSelect()
{
    ...
    for (int i = 0; i < NUM_LEVELS; i++)
    {
        if (levelBubbleLocked[i])
        {
            sf::CircleShape lc = levelBubbleCircle[i];
            ...
            lt.setPosition(levelBubbleX[i] - lb.width / 2.f,
                levelBubbleY[i] - lb.height / 2.f - 8);
            window.draw(lt);
        }
        else
        {
            window.draw(levelBubbleCircle[i]);
            window.draw(levelBubbleNumText[i]);
        }
    }
    ...
    if (showEnemyCards)
    {
        for (int i = 0; i < NUM_CHARS; i++)
        {
            window.draw(enemyCardBg[i]);
            window.draw(enemyCardSpr[i]);
            window.draw(enemyCardNameTxt[i]);
            window.draw(enemyCardDescTxt[i]);
        }
    }
    else
    {
        for (int i = 0; i < NUM_CHARS; i++)
        {
            window.draw(towerCardBg[i]);
            window.draw(towerCardSpr[i]);
            window.draw(towerCardNameTxt[i]);
            window.draw(towerCardDescTxt[i]);
        }
    }
    sf::Text hint;
    hint.setFont(font);
...
void Game::handleMapClick(float mx, float my)
{
    ...
    for (int i = 0; i < towerCount; i++)
    {
        ...
        float dx = towers[i]->get_x() - wp.x;
        float dy = towers[i]->get_y() - wp.y;
...
void Game::handleHUDClick(float mx, float my)
{
    ...
    {
        if (!wave.has_started()) wave.start();
        return;
    }
...
void Game::placeTower(int col, int row)
{
    ...
    Tower* t = nullptr;
    if (pendingTower == HUD::CANNON) 
    {
        t = new CannonTower(pos.x, pos.y);
    }
    else if (pendingTower == HUD::ICE)    
    {
        t = new IceTower(pos.x, pos.y);
    }
    else if (pendingTower == HUD::CANDY)   
    {
        t = new CandyBlaster(pos.x, pos.y);
    }
    else if (pendingTower == HUD::SUNFLOWER) 
    {
        t = new SunflowerTower(pos.x, pos.y);
    }
    else if (pendingTower == HUD::BOMB)   
    {
        t = new BombTower(pos.x, pos.y);
    }
    else 
        return;
...
{
    if (index < 0 || !towers[index])
        return;
    int cost = towers[index]->getUpgradeCost();
    if (gold < cost) { cout << "Not enough gold!\n"; return; }
...
{
    Enemy** enemies = wave.get_enemies();
    int     enemy_count = wave.getenemy_count();
    ...
    for (int i = 0; i < towerCount; i++)
    {
        ...
        if (st) { st->attackAll(enemies, enemy_count); continue; }
        ...
        if (bt)
        {
            ...
            if (wasActive && !bt->isBulletActive())
                bt->explode(enemies, enemy_count);
            Enemy* target = bt->findTarget(enemies, enemy_count);
            if (target) bt->attack(target);
            continue;
        }
        Enemy* target = towers[i]->findTarget(enemies, enemy_count);
...
void Game::startNextWave()
{
    ...
    wave.set_up_wave(currentWave, selectedLevel);
}
